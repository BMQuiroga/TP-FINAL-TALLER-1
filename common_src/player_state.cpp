...
PlayerState::PlayerState(int8_t max_x, int8_t max_y) : direction(2, 0), position(2, 0) {
...
void PlayerState::next_state(const ProtocolRequest &req) {
    if (req.move == MOVE_DOWN) {
        direction[1] = 1;
    } else if (req.move == MOVE_UP) {
        direction[1] = -1;
    } else if (req.move == MOVE_RIGHT) {
        direction[0] = 1;
    } else if (req.move == MOVE_LEFT) {
        direction[0] = -1;
    }
    this->move();
    // switch (req->cmd) {
