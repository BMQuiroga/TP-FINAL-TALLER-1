...
int main(int argc, char *argv[]) { try {
    ...
    Queue<LobbyGameStateResponse> lobby_receiver_q(1000);
    // Clase que contiene el loop principal
    QApplication app(argc, argv);
    ClientLobby client_lobby(std::ref(skt), std::ref(lobby_commands_q), std::ref(lobby_receiver_q));
    client_lobby.start();
    MainWindow w(std::ref(lobby_commands_q), std::ref(lobby_receiver_q));
    ...
    std::string player_name = w.get_player_name();
    ...
    ClientRenderer client_renderer(std::ref(events_q),std::ref(updates_q), player_name);
